...
interface UserTripActiveScreenProps {
  trip: any;
  onGoBack?: () => void;
}

export default function UserTripActiveScreen({
  trip,
  onGoBack = () => {},
}: UserTripActiveScreenProps) {
  const { user } = useAuth();
    ...
    const getRouteCoordinates = async () => {
      ...
      try {
        ...
        if (data.routes && data.routes[0] && data.routes[0].overview_polyline) {
          ...
          console.error(
            "[UserTripActiveScreen] No route found in response:",
            data
          );
        }
      } catch (error) {
        console.error(
          "[UserTripActiveScreen] Error al obtener la ruta:",
          error
        );
      } finally {
      ...
      <View style={styles.bottomCard}>
        <View style={styles.cardContent}>
          ...
          <Text style={styles.infoText}>
            Al finalizar el viaje recuerda no dejar tus pertenencias y calificar
            tu experiencia
          </Text>
